...
describe('Board', function(){
  ...
  describe('getPlayerPiecePos', function(){
    it('Player0のコマは(0, 1)と(0, 2)にある', function(){
      assert.equal(JSON.stringify(board.getPlayerPiecePos(0)), JSON.stringify([p0PiecePos, new Position(0, 2)]));
    })
  })

  describe('getCanMoveVec', function(){
    it('Player0のコマp0Pieceは←と↓に移動できる', function(){
      assert.equal(JSON.stringify(board.getCanMoveVec(0, p0PiecePos)), JSON.stringify([new Position(0, -1), new Position(1, 0)]));
    })
  })
